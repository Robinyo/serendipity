...
- There is **no `managedBy` / manager hierarchy** field on the entity — the access-control model's manager hierarchy is not yet represented in the data model.
- `@Setter` is present — the entity is mutable.
...
| `User.managedBy` (manager hierarchy, on a Serendipity-internal User) | Not present — no User entity in Party Service | The manager hierarchy is a Serendipity-internal concept; if it lives in the Party Service, a `User`-equivalent entity (or a field on an existing user representation) would be needed. If it lives outside the Party Service, the Party Service reads it from the token or from a shared user store. |
...
- **No `User` entity in the Party Service.** The access-control model's `managedBy` field is on a "User" — but the Party Service has no `User` entity. The Party Service's entities are `Party`, `Individual`, `Organisation`, etc. — these are customer-engagement domain objects, not user-management objects. If the manager hierarchy is stored in the Party Service, a user-management entity (or a `managedBy` field on an existing object that represents a user) would be needed. If it is stored elsewhere (e.g. Keycloak, or a separate Serendipity user-management service), the Party Service reads it from the token or from that service.
...
1. **Where does the `User.managedBy` field live?** The Party Service has no `User` entity. If the manager hierarchy is part of the Party Service's data model, a user-management entity (or a `managedBy` on an existing user-representing object) would be needed. If it is in Keycloak or a separate service, the Party Service reads it from the token or from that service at enforcement time.
