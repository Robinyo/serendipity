...
| ON | Pauline Hanson's One Nation | Yes (alias for PHON; same Organisation) |
| AV | Australia's Voice | Yes |
| CLP | Country Liberal Party (Northern Territory) | Yes |
| JLN | Jacqui Lambie Network | Yes |
| UAP | United Australia Party | Yes |
| CA | Centre Alliance | Yes |
| KAP | Katter's Australian Party | Yes |

> **Note:** All 14 party abbreviations found in the APH CSV data are now seeded as `Organisation`s. Each has a `PoliticalParty` enum constant and a dedicated seed class on disk. Members from every one of these parties receive a `Membership` role linking them to their party's Organisation.
...
**The seed's membership switch is the real gate.** Both `Senate` and `HouseOfRepresentatives` seeds use a `switch` on the resolved `PoliticalParty` constant to decide whether to create a **Membership** role linking the individual to a party Organisation. The switch lists all 13 constants that have dedicated seed classes on disk — `AUSTRALIAN_GREENS`, `AUSTRALIAN_LABOR_PARTY`, `LIBERAL_NATIONAL_PARTY_OF_QUEENSLAND`, `LIBERAL_PARTY`, `LIBERAL_PARTY_OF_AUSTRALIA`, `NATIONAL_PARTY_OF_AUSTRALIA`, `PAULINE_HANSONS_ONE_NATION`, `AUSTRALIA_S_VOICE`, `COUNTRY_LIBERAL_PARTY`, `JACQUI_LAMIBE_NETWORK`, `UNITED_AUSTRALIA_PARTY`, `CENTRE_ALLIANCE`, and `KATTER_S_AUSTRALIAN_PARTY` — so members from any of these parties receive a Membership role linking them to their party's Organisation. Senators and members whose abbreviation resolves to `INDEPENDENT` fall through to the `default` branch: no membership role is created, the individual is saved as an Individual + Contact only.

### Extending the enum

If a new party abbreviation appears in a freshly downloaded APH CSV (for example, after a redistribution or a new party registering), add a new enum value with the abbreviation and display name, create a corresponding seed class on disk, and update the `switch` statements in both `Senate.java` and `HouseOfRepresentatives.java`. Then re-run the seed. The enum is the single place that controls which parties are recognized by abbreviation lookup. Abbreviations that remain unrecognized are logged and treated as independents (no membership role created).

For a party to be seeded as an `Organisation`, three things must be true:

- the abbreviation must be in the `PoliticalParty` enum (so the member's `Membership` role is created),
