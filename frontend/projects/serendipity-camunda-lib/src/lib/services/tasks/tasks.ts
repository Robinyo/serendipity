...
export class TasksService extends CollectionService {
  ...
  private readonly tasksApi = '/v2/user-tasks';
  ...
  public search(body: any): Observable<any> {
    ...
    return this.http.post(`${this.tasksApi}/search`, body, this.getDefaultHttpPostOptions()).pipe(
      tap(() => {
  ...
  public form(userTaskKey: string): Observable<any> {
    ...
    return this.http.get(`${this.tasksApi}/${userTaskKey}/form`, this.getDefaultHttpGetOptions()).pipe(
      tap(() => {
  ...
  public assignment(userTaskKey: string, body: any): Observable<any> {
    ...
    return this.http.post(`${this.tasksApi}/${userTaskKey}/assignment`, body, this.getDefaultHttpPostOptions()).pipe(
      tap(() => {
  ...
  public completion(userTaskKey: string, variables: any): Observable<any> {
    ...
    return this.http.post(`${this.tasksApi}/${userTaskKey}/completion`, { variables: variables }, this.getDefaultHttpPostOptions()).pipe(
      tap(() => {
  ...
  pollTask(userTaskKey: string): Observable<TaskResponse> {
    ...
    return this.http.get<TaskResponse>(`${this.tasksApi}/${userTaskKey}`).pipe(
      expand((res) => {
        ...
        return timer(delayForNextAttempt).pipe(
          concatMap(() => this.http.get<TaskResponse>(`${this.tasksApi}/${userTaskKey}`))
        );
      })
    );
