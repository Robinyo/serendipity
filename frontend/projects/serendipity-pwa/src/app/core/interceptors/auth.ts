...
export const authInterceptor: HttpInterceptorFn = (req, next) => {
  ...
  return next(clonedReq).pipe(
    catchError((error: HttpErrorResponse) => {
      // ⚡ CHECK: Is this the initial session verification request?
      const isCheckSessionRequest = url.includes('/api/me');

      // 4. Only trigger Keycloak window breakout if an actual API call drops a 401
      // ⚡ FIXED: Added !isCheckSessionRequest to allow unauthenticated users to stay on the Home page
      if (
        error.status === 401 &&
        (url.startsWith('/api/') || url.startsWith('/v2/')) &&
