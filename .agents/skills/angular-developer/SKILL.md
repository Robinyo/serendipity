...
- **Command:** `pnpm dlx @angular/cli@<requested_version> new <project-name>`
...
- **Command:** `pnpm dlx @angular/cli@latest new <project-name>`
...
- **End-to-End (E2E) Testing**: Detect the installed runner first (`playwright.config.ts` / `cypress.config.ts`) and prefer the [create-e2e-tests](../create-e2e-tests/SKILL.md) skill for writing specs. Read [e2e-testing.md](references/e2e-testing.md) only for setup, and only when no runner is configured or the user asks to change it.
