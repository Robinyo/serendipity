...
**CRITICAL**: You MUST use Angular's new Signal Forms API for all form-related functionality. Prefer non-null initial values ('' for strings, 0 for numbers, [] for arrays); `null` is supported by the API (e.g. `number | null` models) but avoid it unless the domain needs it.
...
export class Example {
  ...
  protected readonly userModel = signal({
    name: '', // prefer non-null, non-undefined initial values
    email: '',
    age: 0, // prefer 0 over null for numbers
    address: {
      street: '',
      city: '',
    },
    hobbies: [] as string[], // prefer [] over null for arrays
  });

  // AVOID unless the domain needs nullable fields:
  // badModel = signal({
  //   name: null,      // prefer ''
  //   age: null,       // prefer 0 (number | null is supported when needed)
  //   items: null      // prefer []
  // });
...
userForm = form(this.userModel, (schemaPath) => {
  ...
  // In v22, all built-in validators (required, pattern, email, min, max, …)
  // accept { when } – use applyWhen to apply whole groups of rules conditionally.
...
## FieldState vs FieldTree: The Parental Requirement

It's important to understand the difference between **FieldTree** (the structure) and **FieldState** (the actual data/signals). (`FormField` is the `[formField]` binding directive, not the structural type.)

**RULE**: You must **CALL** a field as a function to access its state signals (valid, touched, dirty, hidden, etc.).

```ts
// f is a FieldTree (structural)
const f = form(signal({ cat: { name: 'pirojok-the-cat', age: 5 } }));

f.cat.name; // FieldTree: You can't get flags from here!
...
When there are no errors, return `null` or `undefined` (both mean "valid" in v22)
...
| **when option**        | `pattern(p.x, /.../, {when: ...})` (v22: all built-ins) | Use `applyWhen` to gate whole groups of rules                                    |
...
### Conditional validation: `when` vs `applyWhen`

In v22 every built-in validator config (required, pattern, email, min, max, minLength, maxLength, minDate, maxDate) accepts `{ when }`:

```ts
pattern(s.ssn, /^\d{3}-\d{2}-\d{4}$/, { when: isJoint });

// applyWhen is the better fit for gating a GROUP of rules on one condition
applyWhen(s.ssn, isJoint, (ssnPath) => {
  pattern(ssnPath, /^\d{3}-\d{2}-\d{4}$/);
  minLength(ssnPath, 11);
});
```
